...
mfxVersion get_mfx_libhw_version() {
    ...
    mfxVersion test = { 0 };
    //在Win7上检查D3D11
    //由于我们收到了有关桌面组成被切断的报告，D3D11仅限Win8或更高版本
    for (int i = (check_OS_Win8orLater() ? 0 : 1); i < _countof(impl_list); i++) {
        test = get_mfx_lib_version(impl_list[i]);
		printf("[DEBUG]--->test.Major=%d,test.Minor=%d\r\n",test.Major,test.Minor);
...
BOOL check_lib_version(mfxVersion value, mfxVersion required) {
	//printf("[DEBUG]value.Major = %d,required.Major=%d\r\n",value.Major,required.Major);
